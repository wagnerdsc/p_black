...
## Dimensions
  ...
  dimension_group: event_time_tz_sp {
    type: time
    timeframes: [date,day_of_month,day_of_week, minute, day_of_week_index,day_of_year,month,month_name,month_num,fiscal_quarter,fiscal_quarter_of_year,year,time,hour,hour_of_day]
    label: "Event Time TZ-BR"
    sql: TIMESTAMP(FORMAT_TIMESTAMP('%F %T', TIMESTAMP_MICROS(${TABLE}.event_timestamp), "America/Sao_Paulo")) ;;
  ...
  dimension: platform_trat {
    type: string
    sql: CASE WHEN ${TABLE}.platform IN ('ANDROID', 'IOS') THEN 'APP'
...
## Measures
  ...
  measure: total_usuarios_unicos {
    label: "Qtde Usuários"
    type: count_distinct
    sql: CASE WHEN (( (SELECT value.int_value FROM UNNEST(event_params) WHERE key = "engagement_time_msec")  ) > 0) OR (( (SELECT coalesce(cast(value.string_value as INT64),value.int_value) FROM UNNEST(event_params) WHERE key = "session_engaged")  ) = 1) THEN events.user_pseudo_id  ELSE NULL END ;;
  }
  ...
  # Total de sessões únicas definido pelo cliente - 14/11/2023
  measure: total_unique_sessions {
    group_label: "Ecommerce"
    label: "Qtde Sessões"
    description: "Quantidade de sessões únicas"
    type: count_distinct
    sql: CONCAT(${event_param_ga_session_id}, ${user_pseudo_id}) ;;
  }

  ## Item solicitado pelo cliente em 17/11/2023
  measure: taxa_conversao {
    group_label: "Ecommerce"
    label: "Taxa de Conversão"
    type: number
    sql: ${total_transactions} / ${total_unique_sessions};;
    value_format: "0.00%"
  }
